...
/* Record */
    ...
    auto Record::find(DateRange date_range) const noexcept -> std::optional<IndexRange>
    {
        ...
        // Use a PaddedView in order to treat all non-empty vectors as if there
        // are at least three elements. This allows me to remove tedious edge
        // case checks.
        using utils::PaddedView;
        PaddedView<Entry> entries { d_entries };
        auto padding_sz = entries.size() - d_entries.size();
        ...
        else 
        {
            ...
            // allows us to treat the borders as if they were non-border values; there's
        ...
        if (!date_range.max)
            index_max = global_max - padding_sz;
        else 
        {
    ...
    // TODO: (7 Feb. 2025) Check the return types on the `get` implementations.
    ...
    auto Record::metric_dim() const noexcept -> int { return is_empty() ? 0 : static_cast<int>(d_entries[0].metrics().size()); }
